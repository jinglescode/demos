...
function prepareTrainingData(rawData){
  ...
  for(var row_i=0; row_i<rawData.length-1; row_i++){
    trainX.push(rawData[row_i]);
    trainY.push([rawData[row_i+1][0]]);
  }
...
async function predictMarket() {
  ...
  loadFromAlpha('MSFT','demo').then(function(data) {
    ...
    var trainData = prepareTrainingData(data);
    var trainX = trainData[0];
    var trainY = trainData[1];
