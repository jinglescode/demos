...
categories = data_filter(inital_filter_clustering_colors_by)[0];
var nodes = d3.range(_data.length).map(function(i) {
  var cluster_id = categories[ _data[i][inital_filter_clustering_colors_by] ][0];
...
function data_filter(attribute){
  ...
  var total = 0;
  for(i in _data){
    if(!( _data[i][attribute] in categories )){
      categories[_data[i][attribute]] = [Object.keys(categories).length,0];
    }
    categories[_data[i][attribute]][1]++;
    total++;
  }
  num_clusters = Object.keys(categories).length;
...
function change_cluster(attribute){
  ...
  for(i in _data){
    nodes[i].cluster_id = categories[ _data[i][attribute] ][0];
  ...
  for(var i in categories){
    var this_text = svg.append("text")
      .attr("class", "title")
      .attr("fill", "#0074D9")
      .attr("x", gravity_point[categories[i][0]].x-100)
      .attr("y", gravity_point[categories[i][0]].y-50)
      .text(i+": "+categories[i][1]+" ("+parseInt(categories[i][1]/total*1000)/10+"%)");

    for(j in cluster_count_innerclusters[ categories[i][0] ]){
      var percentage = parseInt( cluster_count_innerclusters[ categories[i][0] ][j] / categories[i][1] *1000)/10;
      var tspan1 = this_text.append('tspan')
        .attr("fill", j)
        .attr("dx", -110)
        .attr("dy", 20)
        .text(cluster_count_innerclusters[ categories[i][0] ][j]+'/'+categories[i][1]+' ('+percentage+'%) ');
